...
- Only replay read-only WebDAV methods (`GET`, `HEAD`, `OPTIONS`, `PROPFIND`) after transient failures. A failed `PUT`, `MOVE`, `MKCOL`, or `DELETE` may already have mutated the server; reconcile its operation-specific postcondition instead of blindly retrying.
- Never automatically delete or take over an expired repository lock. Client clocks and leases cannot fence a paused old writer. Keep the confirmation-guarded manual clear action; MOVE acquisition may only determine whether its exact candidate token reached `refs/head.lock`.
...
- Version bumps touch three files together: `manifest.json`, `package.json`, and `versions.json` (maps version → min Obsidian version). Releases are triggered by a tag that must exactly match `manifest.json`'s version with no `v` prefix (e.g. `0.10.1`); CI enforces this.
