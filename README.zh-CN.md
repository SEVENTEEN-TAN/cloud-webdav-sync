...
版本 `0.10.3` 默认仍使用“仅规划”模式，并提供需要用户明确开启的实验性真实同步开关。真实同步不会直接逐文件覆盖 WebDAV 目录，而是使用不可变的 SHA-256 Blob、经过校验的 Commit、完整文件树，以及根据服务器能力选择的 HEAD 更新策略。正确实现条件 ETag 的服务器会使用比较并交换（compare-and-swap）；部分云盘 WebDAV 服务则可使用经过主动探测验证的原子 `MOVE`/禁止覆盖租约。
...
- 普通同步与强制同步共用一个有序操作队列：普通触发会合并，强制操作作为独占屏障逐个执行，所有同步任务均不会重叠。
- 本地待同步队列可合并新建、修改、删除和重命名事件。
- 桌面端状态栏状态，以及跨平台的同步中心窗口。
- 同步中心采用概览、待同步、版本、历史、日志和能力分区；诊断信息不会再默认和操作流程混在同一页面。
- 界面语言可在设置中切换：跟随 Obsidian、简体中文或 English，覆盖设置页、同步中心、冲突解决工作台、通知、日志与状态栏。
...
- 同步中心提供需确认的“清除历史和日志”操作：删除本设备持久化的同步历史并清空运行日志；云端仓库与本地笔记不受影响。
...
- 刷新云端提交历史严格只读：仓库尚未创建时返回空历史；元数据或 HEAD 只缺一项时报告仓库不完整，不会静默初始化或修复。
- MKCOL、owner PUT、MOVE 和持锁 HEAD PUT 即使返回 HTTP 5xx 或连接重置，也会在不重放写请求的前提下轮询并核对精确结果；持续未知或矛盾的状态会保留锁并安全失败，必须确认其他设备已停止后在同步中心手动清除。
...
GitHub Actions 中的 `Build release package` 可以手动触发，也会在推送与 `manifest.json` 版本完全一致的 tag 时触发。例如版本 `0.10.3` 应使用 tag `0.10.3`，不要使用 `v0.10.3`。
...
- 同步中心只列出当前 HEAD 可达的 Commit。强制恢复后的不可达 Commit 可能仍保存在云端；回滚/reflog 界面、垃圾回收和大文件分块尚未实现。仅用于恢复的本地快照 Commit 也可能在垃圾回收功能完成前保持不可达状态。
